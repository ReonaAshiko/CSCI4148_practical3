...
Pneumonia is an infection that causes inflammation and fluid buildup in the lungs, resulting in symptoms such as cough, chest pain, difficulty breathing, and fever.The automatic identification of pneumonia from X-rays provides early detection, improves efficiency, standardizes diagnosis, enhances accessibility to healthcare, aids in triage and resource allocation, and enables screening and monitoring for better patient outcomes.
...
Some file names tell their ID for the patients while the others tell ID and pathogen in addition.
It takes more work if the format of file names are not unified.
...
Normalizing the size of images will be effective for coming process of the research.
...
It coud be use full to compensate the unbalanced part with data argumentation or cut off the data.
...
No, the accuracy are not enough to be said the model is managing to classify. 
...
If there is more dataset, adding the data could be one of the technique to solve this problem.
We can adjust the depth and width of the model to improve this model.
...
The risk of overfitting will increase because the augmented data were made from original data.
...
It is a huge image database that is consisted of approximately 1.2 million images divided into 1,000 classes. It is helpful to use for trainng of DL model, transfer learning and benchmark for performance exaluation.
...
Because it is specifically designed to be lightweight and efficient, making it suitable for deployment on devices with limited computational resources. In addition, it has accuracy and generalization. 

**9_** How many parameters does this network have? How does this compare to better performing networks available in torch?

It has approximately 4.2 million parameters and it is small number compared to ResNet-50 in Torch.
